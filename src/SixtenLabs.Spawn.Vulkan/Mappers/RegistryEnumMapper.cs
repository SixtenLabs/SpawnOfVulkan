  ...
  public class RegistryEnumMapper : Profile
  {
    ...
    private void ConfigureEnumMapping()
    {
      ...
      CreateMap<VkEnumValue, EnumMemberDefinition>()
          .ForMember(dest => dest.Name, opt => opt.MapFrom(m => m.Name))
          .ForMember(dest => dest.DocumentationCommentDefinition, opt => opt.MapFrom(m => AddComment(m.Comment)))
    ...
    private DefinitionName ProcessReturnType(VkConstantValue ree)
    {
      ...
      return new DefinitionName() { Original = type };
    }

    private DocumentationCommentDefinition AddComment(string comment)
    {
      var commentDefinition = new DocumentationCommentDefinition();

      if (!string.IsNullOrEmpty(comment))
      {
        commentDefinition.CommentDefinitions.Add(new CommentDefinition() { Comment = comment });
      }
